...
fsubset <- expand.grid(model=c('ngo', 'patient'), subset=c('HIV', 'Healthy','TFH016', 'TFH017', 'TFH023', 'TFH041'), stringsAsFactors=FALSE)
...
samp10 <- subset(samp10[,colnames(samp1)], SafeCS %in% c('pp'))
manyfits <- list()
manystab <- list()
for(i in seq_len(nrow(fsubset))){
    ...
    if(mdl=='SafeCS'){
        ...
        fixed <- model.matrix(form, cData(thiss1), contrasts=list(PatientID='contr.sum', SafeCS='contr.sum'))
        fixed10 <- model.matrix(form, cData(thiss10), contrasts=list(PatientID='contr.sum', SafeCS='contr.sum'))
    } else if(mdl=='ngo'){
                fixed <- model.matrix(~scale(ngeneson), cData(thiss1), contrasts=list(PatientID='contr.sum'))
                fixed10 <- model.matrix(~scale(ngeneson), cData(thiss10), contrasts=list(PatientID='contr.sum'))

    } else if(mdl=='patient'){
        fixed <- model.matrix(~scale(ngeneson)+PatientID, cData(thiss1), contrasts=list(PatientID='contr.sum'))
        fixed10 <- model.matrix(~scale(ngeneson)+PatientID, cData(thiss10), contrasts=list(PatientID='contr.sum'))
    }
    ...
    out10 <- setNames(fitSomeModels(thiss10, 'gaussian', fixed=fixed10, parallel=T, nlambda=50, lambda.min.ratio=.3), 'gaussian10')
    ee10 <- precenter(exprs(thiss10))
