...
!(example_graph.png)[A graph and its probability distribution.
C directly affects A, but not D.]
...
covariates = cbind(1, scale(shalek2014_list$cdata$ngeneson))
...
hurdle_fit = fitHurdle(exp1h_trans,  fixed=covariates, parallel=FALSE, control=list(debug=0))
```
...
library(igraph)
edge_bic <- seq(10, BIC_etc[isMax==TRUE,trueEdges], length.out=4)
...
* **Differential coexpression**. Most of the infrastructure is in place to use these procedures to look for
  ...
  the functions that would need to be overrided and used.
