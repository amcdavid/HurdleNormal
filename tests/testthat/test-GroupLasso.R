...
test_that('Can fit', {
    ...
    browser()
    paths1 <- cgpaths(y.zif, this.model, lambda=lambda, control=list(tol=1e-7, maxrounds=2000, debug=1, method='proximal'))
    paths2 <- cgpaths(y.zif, this.model, lambda=lambda, control=list(tol=1e-7, maxrounds=2000, debug=1, method='block'))
